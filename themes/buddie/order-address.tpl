...
{/if}
	<div class="addresses clearfix">
		...
		{if $addresses|@count > 1}
			...
			{else}
				<a style="margin-left: 221px;" href="{$link->getPageLink('address', true, NULL, "back={$back_order_page}?step=1&select_address=1{if $back}&mod={$back}{/if}")}" title="{l s='Add'}" class="button_large button"><span class="in_button">{l s='Add a new address'}</span></a>
			{/if}
		</p>
		<div class="clearfix">
		...
		<p class="address_add submit">
			<a href="{$link->getPageLink('address', true, NULL, "back={$back_order_page}?step=1{if $back}&mod={$back}{/if}")}" title="{l s='Add'}" class="button_large button"><span class="in_button">{l s='Add a new address'}</span></a>
		</p>
		{if !$opc}
...
{if !$opc}
	<p class="cart_navigation submit">
		...
		<a href="{$link->getPageLink($back_order_page, true, NULL, "step=0{if $back}&back={$back}{/if}")}" title="{l s='Previous'}" class="button"><span class="in_button">{l s='Previous'}</span></a>
		<span class="out_button"><input type="submit" name="processAddress" value="{l s='Next'} &raquo;" class="exclusive" /></span>
	</p>
</form>
{else}
</div>

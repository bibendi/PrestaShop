...
{if !$opc}<h1>Для завершения заказа выберите способ оплаты</h1>{else}<h2><span>3</span> Для завершения заказа выберите способ оплаты</h2>{/if}
...
{if !$opc}
	<p class="cart_navigation payment"><a href="{$link->getPageLink('order', true, NULL, "step=2")}" title="{l s='Previous'}" class="button"><span class="in_button">{l s='Previous'}</span></a></p>
{else}
	</div>
{/if}
</div>

...
<h1>{if !isset($email_create)}{l s='Log in'}{else}Регистрация{/if}</h1>
    ...
    <form action="{$link->getPageLink('authentication', true)}" method="post" id="create-account_form" class="std">
        <fieldset>
            ...
            <div class="form_content clearfix">
                <p class="title_block">Введите ваш адрес электронной почты.</p>
                ...
                <p class="submit">
                    ...
                    <input type="submit" id="SubmitCreate" name="SubmitCreate" class="button_large" value="Регистрация" />
...
{else}
    ...
    <fieldset class="account_creation">
        ...
        <p class="required password">
            ...
            <span class="form_info">(Не менее 5 символов)</span>
        </p>
        {*
        <p class="select">
            <span>{l s='Date of Birth'}</span>
    ...
    <p class="cart_navigation required submit">
        ...
        {if isset($back)}<input type="hidden" class="hidden" name="back" value="{$back|escape:'htmlall':'UTF-8'}" />{/if}
        <input type="submit" name="submitAccount" id="submitAccount" value="Регистрация" class="exclusive" />
        <span><sup>*</sup>{l s='Required field'}</span>
    </p>
    </form>
{/if}
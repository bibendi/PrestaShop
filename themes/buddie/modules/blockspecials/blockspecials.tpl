...
{if $special}
		<ul class="products">			
			<li>
				...
				<div class="product_desc">{$special.description_short|strip_tags|truncate:60:'...'}</div>
				...
				<div>
					...
					<a class="exclusive ajax_add_to_cart_button" rel="ajax_id_product_{$special.id_product}" href="{$link->getPageLink('cart.php')}?qty=1&amp;id_product={$special.id_product}&amp;token={$static_token}&amp;add" title="{l s='Add to cart' mod='blockspecials'}"><span class="addtocard">{l s='Add to cart' mod='blockspecials'}</span></a>
						{else}
						<span class="exclusive"><span class="addtocard">{l s='Out of stock' mod='blockspecials'}</span></span>
					{/if}
				</div>
			</li>
		</ul>
